...
export default async function Page({ params }) {
  ...
  const items = await Promise.all(projectCategories.items.map(async (data) => {
    ...
    return <Category key={data.category} categoryData={data} projectID={projectID} />
  }))
  ...
  return (
    <>
      ...
      <Link href="/" passHref>
              <span className="text-black bg-yellow-500 px-4 py-2 rounded-full cursor-pointer hover:bg-yellow-600">Home</span>
            </Link>
